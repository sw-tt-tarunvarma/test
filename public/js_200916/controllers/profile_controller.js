...
.controller('ProfileController', ["$scope", "$state", "$location", "$rootScope", "$http", "$timeout", "getBaseUrl", "$q", "$stateParams", "ngDialog", "$filter", function($scope, $state, $location, $rootScope, $http, $timeout, getBaseUrl, $q, $stateParams, ngDialog, $filter, base64Converter) {
  ...
  $scope.$on('$viewContentLoaded', function () 
  {
    ...
    $scope.displayNotificcation = false;
    ...
    /* code added by dhaval thaker <dhaval.thaker@softwebsolutions.com > Get Spaces */
    /**
      * @module : profile
      * @desc   : Get profile
    ...
    */
      $rootScope.updatePeople = function(id) {      
      var name = $rootScope.userData.name;
      if (name && name.trim()) {
        ...
        if($rootScope.userData.newimage && $rootScope.userData.newimage.filename) {
          var allowMimeType = ['image/jpeg','image/png'];
          var checkMimeType = jQuery.inArray( $rootScope.userData.newimage.filetype, allowMimeType );
          if (checkMimeType < 0) {
        ...
        if(isSaveSpace) {
          $rootScope.userData.id = id;
          $rootScope.userData.name = name.trim();
    ...
    */
    ...
    //added by DT <dhaval.thaker@softwebsolutions.com>
