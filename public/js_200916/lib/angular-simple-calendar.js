  ...
  return {
    ...
    template: '<div class="row"><div class="col-md-6"><div class="custom-header clearfix"><span class="custom-today">Today - {{curdate | date:"fullDate"}}</span><h2>Choose a Calendar<span>Which calendar should Robin use to manage this space?</span></h2></div></div><div class="col-md-6"><button class="btn btn-inline btn-secondary-outline  event_button" type="button"><i class="fa fa-plus"></i> Add a Calendar</button></div><div class="col-md-12"><span class="custom-month-year">{{ selectedMonth }} '+' {{ selectedYear }}</span></div></div> <div class="fc-calendar-container"><div class="fc-calendar"><div class="fc-head"><div ng-repeat="day in WEEKDAYS" class="weekday fc-head">{{ day }}</div></div><div class="fc-body">'+
     '<div>' +
      ...
      '<div><span class="fc-date">{{ date.day || "&nbsp;" }}</span><span class="in-use" ng-if="date.testevents.length > 0"><h4 ng-repeat="te in date.testevents track by $index" ng-click="editEventPopup(te.eventid)">{{ te.title }}</h4></span><span class="fc-weekday" style="float:right" ng-if="date.day"></span></div>' +
      ' </div>' +
      '</div>' +
      '</div>' +
    '</div></div></div>',
