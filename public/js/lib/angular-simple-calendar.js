angular.module('500tech.simple-calendar', []).directive('simpleCalendar', function () {
  return {
    ...
    template: '<div class="row"><div class="col-md-6"><div class="custom-header clearfix"><span class="custom-today">Today - {{curdate | date:"fullDate"}}</span><h2>Choose a Calendar<span>Which calendar should Robin use to manage this space?</span></h2></div></div><div class="col-md-6"><button class="btn btn-inline btn-secondary-outline  event_button" type="button"><i class="fa fa-plus"></i> Add a Calendar</button></div><div class="col-md-12" style="text-align:center;"><div class="move-month prev-month" ng-click="prevMonth()"><span ng-show="allowedPrevMonth()">Prev</span></div><span class="custom-month-year">{{ selectedMonth }} '+' {{ selectedYear }}</span><div class="move-month next-month" ng-click="nextMonth()"><span ng-show="allowedNextMonth()">Next</span></div></div></div> <div class="fc-calendar-container"><div class="fc-calendar"><div class="fc-head"><div ng-repeat="day in WEEKDAYS" class="weekday fc-head">{{ day }}</div></div><div class="fc-body">'+
     '<div>' +
      ...
      '<div><span class="fc-date">{{ date.day || "&nbsp;" }}</span><span class="fc-weekday" style="float:right" ng-if="date.day"></span><span ng-if="date.testevents.length > 0" style="position:absolute;left:10px;" ng-click="showevent(date.testevents)">+{{date.testevents.length}} more</span></div><span ng-if="selectedDate <= date.day ">Add New </span>' +
      ' </div>' +
      '</div>' +
      '</div>' +
    '</div></div></div>',
    ...
    controller: ['$scope', '$rootScope', "$http", "$timeout", 'ngDialog','$filter', function ($scope, $rootScope, $http, $timeout, ngDialog, $filter) {
      ...
      $scope.calendarSelectDate = $scope.curdate.getFullYear()+'-'+$scope.curdate.getMonth()+'-'+$scope.curdate.getDate();
      ...
      $scope.showevent = function (data) {
        ...
        $rootScope.eventdata.date = new Date($rootScope.eventdata[0].Date.date);
        console.log($rootScope.eventdata);
         ngDialog.open({ 
          template: 'partials/event_list.html',
      ...
      */
      $scope.addEventPopup = function (date) {
        ...
        $rootScope.eventPopupData.locationid = $rootScope.spaceDetailData.id;
        $rootScope.eventPopupData.spaceCapacity = parseInt($rootScope.spaceDetailData.capacity);
        var curDate = $rootScope.eventPopupData.currentDate.getDate();
        var calenderDate = $rootScope.eventPopupData.calDate.getDate();
        var sltDate = $rootScope.eventPopupData.selectedDate.getDate();
        $rootScope.eventPopupData.btndisabled = 'true'
        console.log(curDate);
        console.log(calenderDate);
        console.log(sltDate);
        if(curDate <= calenderDate || sltDate < calenderDate)
        {
          console.log('Current');
          $rootScope.eventPopupData.btndisabled = ''
        }
        else
        {
      ...
      /**
        ...
        * @author : Softweb solutions - Alpeshsinh Solanki
      */
      $rootScope.editEventPopup = function ($event,eventid) {
      ...
      */
      ...
      bindEvent = function (date) {
        ...
        $scope.events.forEach(function(event) {
          ...
          $scope.pastdates = date.year+'-'+date.month+'-'+date.day;
        console.log($scope.pastdates);
          if (date.year === event.date.getFullYear() && date.month === event.date.getMonth() && date.day === event.date.getDate()) {
            date.event = event;
            // Added by Softweb
            date.testevents.push({title : date.event.title, eventid : date.event.eventid,Date:date.event});
            //
          }
        });
      };
      ...
      calculateSelectedDate = function () {
        ...
        $scope.selectedMonthNUMBER = MONTHSNUMBER[$scope.options._defaultDate.getMonth()];
