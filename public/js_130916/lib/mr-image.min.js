...
app.directive("mrImage", function() {
    ...
    return {
        ...
        template: '<div mr-image-selector mr-model="selector" mr-aspect-ratio="aspectRatio" style="height: {{scaleValue(height, scale) + \'px\'}}; width: {{scaleValue(width, scale) + \'px\'}}"></div><div mr-image-drawer mr-model="drawer" style="height: {{scaleValue(height, scale) + \'px\'}}; width: {{scaleValue(width, scale) + \'px\'}}"></div><img ng-src="{{src}}" width="{{scaleValue(width, scale)}}" height="{{scaleValue(height,scale)}}">',
        link: function(a, b) {

            a.$watch('src', function(newValue, oldValue) { 
              if (newValue){
            ...
            function c(c) {
                ...
                a.image.onload = function() {
                    ...
                    a.$apply(function() {
                        a.height = a.height || a.image.height,
                        a.width = a.width || a.image.width,
                        a.scale = angular.isUndefined(a.scale) && angular.isDefined(a.maxWidth) ? a.maxWidth >= a.width ? 1 : a.maxWidth / a.width : a.scale || 1,
                        b.css("width", a.scaleValue(a.width, a.scale) + "px"), b.css("height", a.scaleValue(a.height, a.scale) + "px")
                    })
                },
                a.image.src = c
            }
    ...
    return {
        ...
        template: "<div ng-repeat=\"rect in rects\" style=\"position: absolute;cursor: pointer;top:    {{ rect.y1 }}px;left:   {{ rect.x1 }}px;width:  {{ (rect.x2 - rect.x1)}}px;height: {{ (rect.y2 - rect.y1)}}px;border: {{ rect.stroke || 3 }}px solid {{ rect.color || '#F00' }};background-color: {{ increaseBrightness(rect.color || '#F00', '0.3') }}\"ng-style='rectStyle'ng-mouseenter=\"rectStyle = { 'background-color': increaseBrightness(rect.color || '#F00', '0.1') }\"ng-mouseout=\"rectStyle =   { 'background-color': increaseBrightness(rect.color || '#F00', '0.3' ) }\"></div>",
        link: function(a) {
       
        a.$watch('rects', function(newValue, oldValue) { 
        if (newValue){
    ...
    return {
        ...
        b.$watch('selector', function(newValue, oldValue) { 
        if (newValue){
            ...
            var H = angular.element(document),
                I = angular.element('<div class="mr-box"><div class="mr-line top"></div><div class="mr-line bottom"></div><div class="mr-line left"></div><div class="mr-line right"></div></div>'),
