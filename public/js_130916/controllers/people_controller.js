...
.controller('PeopleController', ["$scope", "$location", "$rootScope", "$http","$timeout", "getBaseUrl","$q","ngDialog","$filter","base64Converter", "$state",function($scope, $location, $rootScope, $http, $timeout, getBaseUrl, $q, ngDialog, $filter, base64Converter, $state) {
  ...
  */
  $scope.getRoomList = function(notificationType, userID){
    ...
    {
      $http.post($scope.baseurl+"/people/getRoomList")
      .success(function(result, status, headers, config) {
